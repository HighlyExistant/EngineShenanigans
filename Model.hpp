...
namespace cow 
{
	...
	*/
	...
	public:
		...
		Model2DComponent(Device& device, std::vector<Vertex2DRGB> vertices2d)
		...
		Model2DComponent(Device& device, uint32_t count, Vertex2DRGB* vertices2d)
		...
		static inline Model2DComponent createStagedModel2D(Device& device, std::vector<Vertex2DRGB> vertices2d)
		{
	...
	private:
		Model2DComponent(std::vector<Vertex2DRGB> vertices2d, Device& device)
			: m_ref_device{ device }, vertexCount{ static_cast<uint32_t>(vertices2d.size()) }
		{
			assert(vertexCount >= 3 && "Vertex Count must be atleast 3");
			VkDeviceSize bufferSize = sizeof(vertices2d[0]) * vertexCount;
